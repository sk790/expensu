...
export default function AddExpenseScreen({ route, navigation }) {
  ...
  const [attachment, setAttachment] = useState(() => {
    if (isEditing && expenseData && expenseData.attachment) {
      return expenseData.attachment;
    }
    return null;
  });
    ...
    try {
      if (isEditing && expenseData) {
        await groupService.editExpense(
          ...
          parseFloat(amount),
          finalSplitData,
          description.trim(), paidBy, selectedCategory, attachment
        );
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
        showAlert({
  ...
  const pickAttachment = async () => {
    ...
    if (status !== 'granted') {
      showAlert({
        ...
        message: "We need library permissions to let you select a receipt/attachment."
      });
      return;
    }
    try {
      ...
      console.log("Failed to pick attachment:", error);
      showAlert({
        type: "error",
        title: "Selection Failed",
        message: "Failed to pick the image. Please try again."
      });
    }
  };
  ...
  const takePhotoAttachment = async () => {
    ...
    if (status !== 'granted') {
      showAlert({
        type: "error",
        title: "Permission Denied",
        message: "We need camera permissions to let you snap a photo of the receipt."
      });
      return;
    }
    try {
  ...
  const pickPdfAttachment = async () => {
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        setAttachment(base64Pdf);
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      }
    } catch (error) {
      console.log("Failed to pick PDF:", error);
      showAlert({
        type: "error",
        title: "Selection Failed",
        message: "Failed to pick the PDF document. Please try again."
      });
    }
  };

  const pickVideoAttachment = async () => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    try {
      const result = await DocumentPicker.getDocumentAsync({
        type: "video/*",
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        const base64Video = `data:${mimeType};base64,${base64Data}`;
        setAttachment(base64Video);
        Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
      }
    } catch (error) {
      ...
      showAlert({
        type: "error",
        title: "Selection Failed",
        message: "Failed to pick the video file. Please try again."
      });
    }
  };
            ...
            {attachment ? (
              isPdfAttachment(attachment) ? (
                <View style={styles.pdfPreviewContainer}>
                  <View style={[styles.pdfIconCircle, { backgroundColor: "#EF4444" + "12" }]}>
                    <Ionicons name="document-text" size={38} color="#EF4444" />
                  </View>
                  <View style={styles.pdfTextContainer}>
                    <Text style={styles.pdfNameText}>Receipt Document (PDF)</Text>
                    <Text style={styles.pdfSizeText}>Click Save to persist PDF document</Text>
                  </View>
                  <View style={styles.attachmentOverlayButtonsPDF}>
                    <TouchableOpacity
                      style={[styles.attachmentOverlayButton, styles.attachmentDeleteButton]}
                    ...
                    <TouchableOpacity
                      style={[styles.attachmentOverlayButton, styles.attachmentEditButton]}
                      onPress={pickPdfAttachment}
                      activeOpacity={0.7}
                    >
                      <Ionicons name="document-text" size={16} color={COLORS.white} />
                      <Text style={styles.attachmentButtonText}>Change</Text>
                    </TouchableOpacity>
                  </View>
                </View>
              ) : isVideoAttachment(attachment) ? (
                <View style={styles.videoPreviewContainer}>
                  <View style={[styles.videoIconCircle, { backgroundColor: COLORS.primary + "12" }]}>
                    <Ionicons name="play-circle" size={38} color={COLORS.primary} />
                  </View>
                  <View style={styles.pdfTextContainer}>
                    <Text style={styles.pdfNameText}>Receipt Video (MP4/MOV)</Text>
                    <Text style={styles.pdfSizeText}>Click Save to persist receipt video</Text>
                  </View>
                  <View style={styles.attachmentOverlayButtonsPDF}>
                    <TouchableOpacity
                      style={[styles.attachmentOverlayButton, styles.attachmentDeleteButton]}
                ...
                <View style={styles.attachmentPreviewContainer}>
                  <Image source={{ uri: attachment }} style={styles.attachmentPreviewImage} />
                  <View style={styles.attachmentOverlayButtons}>
                    <TouchableOpacity
                      style={[styles.attachmentOverlayButton, styles.attachmentDeleteButton]}
                      onPress={removeAttachment}
                      activeOpacity={0.7}
              ...
              <View style={styles.attachmentPickerRow}>
                <TouchableOpacity
                  style={styles.pickerActionBox}
                  onPress={pickAttachment}
                  activeOpacity={0.7}
                >
                  <View style={[styles.pickerIconWrapper, { backgroundColor: COLORS.primary + "12" }]}>
                    <Ionicons name="image-outline" size={18} color={COLORS.primary} />
                  </View>
                  <Text style={styles.pickerActionTitle}>Gallery</Text>
                  <Text style={styles.pickerActionSubtitle}>Photo</Text>
                </TouchableOpacity>

                <TouchableOpacity
                  style={styles.pickerActionBox}
                  onPress={takePhotoAttachment}
                  activeOpacity={0.7}
                >
                  <View style={[styles.pickerIconWrapper, { backgroundColor: "#10B981" + "12" }]}>
                    <Ionicons name="camera-outline" size={18} color="#10B981" />
                  </View>
                  <Text style={styles.pickerActionTitle}>Camera</Text>
                  <Text style={styles.pickerActionSubtitle}>Photo</Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <View style={[styles.pickerIconWrapper, { backgroundColor: "#EF4444" + "12" }]}>
                    <Ionicons name="document-text-outline" size={18} color="#EF4444" />
                  </View>
                  <Text style={styles.pickerActionTitle}>PDF</Text>
                  <Text style={styles.pickerActionSubtitle}>Doc</Text>
                </TouchableOpacity>
                ...
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <View style={[styles.pickerIconWrapper, { backgroundColor: "#8B5CF6" + "12" }]}>
                    <Ionicons name="videocam-outline" size={18} color="#8B5CF6" />
                  </View>
                  <Text style={styles.pickerActionTitle}>Video</Text>
                  <Text style={styles.pickerActionSubtitle}>Clip</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
